...
static int wchar_string_to_cp850_string(char *out, const wchar_t *in, unsigned int out_size)
{
    ...
    for (i = 0; i < out_size-1 && in[i]; ++i) {
        ...
        if (j == 0x80) {
            fprintf(stderr, "Cannot convert input character 0x%04x to 'CP850': strerror(EILSEQ)\n", (unsigned int)in[i]);
            return 0;
        }
    }
    if (in[i]) {
...
static int local_string_to_cp850_string(char *out, const char *in, unsigned int out_size)
{
    ...
    if (!wcs) {
        fprintf(stderr, "Cannot convert input string '%s' to 'CP850': strerror(ENOMEM)\n", in);
        return 0;
    }
...
static int iconv_init_codepage(int codepage, const char *local, iconv_t *to_local, iconv_t *from_local)
{
    ...
    *to_local = iconv_open(local, codepage_name);
    ...
    if (*to_local == (iconv_t) - 1)
        fprintf(stderr, "Cannot initialize conversion from codepage %d to %s: strerror(%d)\n", codepage, local, errno);
    snprintf(codepage_name, sizeof(codepage_name), "CP%d", codepage);
    *from_local = iconv_open(codepage_name, local);
    if (*from_local == (iconv_t) - 1)
        fprintf(stderr, "Cannot initialize conversion from %s to codepage %d: strerror(%d)\n", local, codepage, errno);
...
int local_string_to_dos_string(char *out, char *in, unsigned int out_size)
{
    ...
    if (ret == (size_t)-1) {
        if (errno == E2BIG)
            ...
            fprintf(stderr, "Cannot convert input sequence '\\x%.02hhX' from codeset '%s' to 'CP%d': strerror(%d)\n",
                    *pin, nl_langinfo(CODESET), used_codepage, errno);
...
int dos_string_to_wchar_string(wchar_t *out, char *in, unsigned int out_size)
{
    ...
    if (ret == (size_t)-1) {
        if (errno == E2BIG)
            ...
            fprintf(stderr, "Cannot convert input sequence '\\x%.02hhX' from 'CP%d': strerror(%d)\n",
                    *pin, used_codepage, errno);
...
int wchar_string_to_dos_string(char *out, wchar_t *in, unsigned int out_size)
{
    ...
    if (ret == (size_t)-1) {
        if (errno == E2BIG)
            ...
            fprintf(stderr, "Cannot convert input character '%lc' to 'CP%d': strerror(%d)\n",
                    (wint_t)*(wchar_t *)pin, used_codepage, errno);
