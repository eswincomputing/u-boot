...
static bool is_uv_swap(uint32_t bus_format, uint32_t output_mode)
{
	...
	if ((bus_format == MEDIA_BUS_FMT_YUV8_1X24 ||
	    ...
	    (output_mode == ESWIN_OUT_MODE_AAAA ||
	     output_mode == ESWIN_OUT_MODE_P888))
		return true;
	else
		return false;
}

static int eswin_vop_init_gamma(struct vop *vop, struct display_state *state)
{
...
static int eswin_vop_preinit(struct display_state *state)
{
...
static int eswin_vop_init(struct display_state *state)
{
	struct crtc_state *crtc_state = &state->crtc_state;
	struct connector_state *conn_state = &state->conn_state;
	struct drm_display_mode *mode = &conn_state->mode;
	...
	vop->grf = syscon_get_first_range(ESWIN_SYSCON_GRF);
	if (vop->grf <= 0)
	...
	eswin_vop_init_gamma(vop, state);
	...
	case DRM_MODE_CONNECTOR_DSI:
		...
		VOP_CTRL_SET(vop, mipi_dual_channel_en,
			!!(conn_state->output_type & ESWIN_OUTPUT_DSI_DUAL_CHANNEL));
		VOP_CTRL_SET(vop, data01_swap,
			!!(conn_state->output_type & ESWIN_OUTPUT_DSI_DUAL_LINK) ||
	...
	if (conn_state->output_mode == ESWIN_OUT_MODE_AAAA &&
	...
	VOP_CTRL_SET(vop, dclk_ddr,
		     conn_state->output_mode == ESWIN_OUT_MODE_YUV420 ? 1 : 0);
...
static int eswin_vop_setup_csc_table(struct display_state *state)
{
	struct crtc_state *crtc_state = &state->crtc_state;
	struct connector_state *conn_state = &state->conn_state;
...
static int eswin_vop_set_plane(struct display_state *state)
{
	struct crtc_state *crtc_state = &state->crtc_state;
	const struct eswin_crtc *crtc = crtc_state->crtc;
	const struct vop_data *vop_data = crtc->data;
	 ...
	 */
	if (crtc_state->format == ESWIN_FMT_RGB888 && VOP_MAJOR(vop_data->version) == 3)
	...
	eswin_vop_setup_csc_table(state);
...
static int eswin_vop_prepare(struct display_state *state)
{
	return 0;
}

static int eswin_vop_enable(struct display_state *state)
{
	struct crtc_state *crtc_state = &state->crtc_state;
	struct vop *vop = crtc_state->private;
...
static int eswin_vop_disable(struct display_state *state)
{
	struct crtc_state *crtc_state = &state->crtc_state;
	struct vop *vop = crtc_state->private;
...
static int eswin_vop_fixup_dts(struct display_state *state, void *blob)
{
#if 0
	struct crtc_state *crtc_state = &state->crtc_state;
...
static int eswin_vop_send_mcu_cmd(struct display_state *state,
				     u32 type, u32 value)
{
	struct crtc_state *crtc_state = &state->crtc_state;
	struct vop *vop = crtc_state->private;
...
const struct eswin_crtc_funcs eswin_vop_funcs = {
	.preinit = eswin_vop_preinit,
	.init = eswin_vop_init,
	.set_plane = eswin_vop_set_plane,
	.prepare = eswin_vop_prepare,
	.enable = eswin_vop_enable,
	.disable = eswin_vop_disable,
	.fixup_dts = eswin_vop_fixup_dts,
	.send_mcu_cmd = eswin_vop_send_mcu_cmd,
};
